...
// The context value key used for holding API instances
const apisKey = Symbol('Context.apis');
...
export class RootContext implements Context {
  ...
  /**
   * {@inheritdoc Context.withApis}
   */
  withApis(...apis: Array<AnyApiRef | AnyApiFactory>): Context {
    return this.withValue<ContextApis>(apisKey, previous =>
      createApis(apis, previous),
    );
  }

  /**
   * {@inheritdoc Context.api}
   */
  api<Api>(ref: ApiRef<Api>): Api | undefined {
    const apis = this.value<ContextApis>(apisKey);
    return apis?.get(ref.id) as Api | undefined;
  }
